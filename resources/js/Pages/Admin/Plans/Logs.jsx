...
import { ArrowLeft, History, User, Calendar, ArrowRight, DollarSign } from 'lucide-react';

export default function Logs({ plan, logs }) {
    return (
        <AdminLayout header={<h2 className="text-xl font-semibold leading-tight text-gray-800 dark:text-gray-200">Price Audit Log</h2>}>
            <Head title={`Audit Log - ${plan.name}`} />
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-white/5 transition-colors">
                                ...
                                <th className="px-6 py-4 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider text-center">Currency</th>
                            ...
                            {logs.length > 0 ? logs.map((log) => (
                                <tr key={log.id} className="hover:bg-slate-50/50 dark:hover:bg-white/5 transition-colors group">
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="flex flex-col gap-2 items-center">
                                            <span className="px-2 py-0.5 rounded-lg bg-emerald-50 text-emerald-700 text-[10px] font-bold uppercase border border-emerald-100 dark:bg-emerald-900/30 dark:text-emerald-400">BDT</span>
                                            <span className="px-2 py-0.5 rounded-lg bg-blue-50 text-blue-700 text-[10px] font-bold uppercase border border-blue-100 dark:bg-blue-900/30 dark:text-blue-400">USD</span>
                                        </div>
                                    </td>
                                ...
                                <tr>
                                    <td colSpan="4" className="px-6 py-12 text-center text-slate-500 dark:text-slate-400">
                                        No pricing history found for this plan.
                                    </td>
                                </tr>
                            )}
                        </tbody>
